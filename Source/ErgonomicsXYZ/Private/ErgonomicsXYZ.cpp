...
bool FErgonomicsXYZModule::TraverseWidgetTree(
	const TSharedPtr<SWidget>& TraverseWidget, TSharedPtr<SWidget>& DockingTabWell, int32 Depth)
{
	...
	if (TraverseWidget->GetTypeAsString() == "SDockingTabWell")
	{
		...
		UE_LOG(ErgonomicsXYZModuleSub, Display, TEXT("%s[Depth: %d] Found SDockingTabWell: %s"), *FString::ChrN(Depth * 2, ' '),
			Depth, *TraverseWidget->ToString());
		return true;
	}
...
{
	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[0], "MoveToTabLast", "Focus Last Tab", "Moves to the last tab",
		EUserInterfaceActionType::Button, TabChords[0]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[1], "MoveToTab1", "Focus Tab 1", "If exists, draws attention to major tab 1",
		EUserInterfaceActionType::Button, TabChords[1]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[2], "MoveToTab2", "Focus Tab 2", "If exists, draws attention to major tab 2",
		EUserInterfaceActionType::Button, TabChords[2]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[3], "MoveToTab3", "Focus Tab 3", "If exists, draws attention to major tab 3",
		EUserInterfaceActionType::Button, TabChords[3]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[4], "MoveToTab4", "Focus Tab 4", "If exists, draws attention to major tab 4",
		EUserInterfaceActionType::Button, TabChords[4]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[5], "MoveToTab5", "Focus Tab 5", "If exists, draws attention to major tab 5",
		EUserInterfaceActionType::Button, TabChords[5]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[6], "MoveToTab6", "Focus Tab 6", "If exists, draws attention to major tab 6",
		EUserInterfaceActionType::Button, TabChords[6]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[7], "MoveToTab7", "Focus Tab 7", "If exists, draws attention to major tab 7",
		EUserInterfaceActionType::Button, TabChords[7]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[8], "MoveToTab8", "Focus Tab 8", "If exists, draws attention to major tab 8",
		EUserInterfaceActionType::Button, TabChords[8]);

	UI_COMMAND_EXT(MainFrameContext.Get(), CommandInfoTabs[9], "MoveToTab9", "Focus Tab 9", "If exists, draws attention to major tab 9",
