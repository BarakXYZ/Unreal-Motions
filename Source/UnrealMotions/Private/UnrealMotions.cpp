...
// DEFINE_LOG_CATEGORY_STATIC(UnrealMotionsModuleSub, NoLogging, All);  // Shipping
DEFINE_LOG_CATEGORY_STATIC(UnrealMotionsModuleSub, Log, All); // Development

#define LOCTEXT_NAMESPACE "FUnrealMotionsModule"

void FUnrealMotionsModule::StartupModule()
{
	FHelpers::SetPluginConfigFile();
...
void FUnrealMotionsModule::RemoveDefaultCommand(FInputBindingManager& InputBindingManager, FInputChord Command)
{
...
void FUnrealMotionsModule::MapTabCommands(TSharedRef<FUICommandList>& CommandList)
{
...
void FUnrealMotionsModule::OnMoveToTab(int32 TabIndex)
{
	UE_LOG(UnrealMotionsModuleSub, Display, TEXT("OnMoveToTab1"));
	...
	if (ActiveWindow.IsValid())
	{
		...
		if (!Found)
		{
			UE_LOG(UnrealMotionsModuleSub, Warning, TEXT("No SDockingTabWell found in the active window."));
		}
		else
		{
	...
	{
		UE_LOG(UnrealMotionsModuleSub, Warning, TEXT("No active top-level window found."));
	}
}

bool FUnrealMotionsModule::TraverseWidgetTree(const TSharedPtr<SWidget>& TraverseWidget,
	TSharedPtr<SWidget>& DockingTabWell, int32 Depth)
{
	// Log the current widget and depth
	UE_LOG(UnrealMotionsModuleSub, Display, TEXT("%s[Depth: %d] Checking widget: %s"),
	...
	if (TraverseWidget->GetTypeAsString() == "SDockingTabWell")
	{
		...
		UE_LOG(UnrealMotionsModuleSub, Display, TEXT("%s[Depth: %d] Found SDockingTabWell: %s"),
...
void FUnrealMotionsModule::FocusTab(const TSharedPtr<SWidget>& DockingTabWell, int TabIndex)
{
...
void FUnrealMotionsModule::InitTabNavInputChords(bool bCtrl, bool bAlt, bool bShift, bool bCmd)
 ...
 */
void FUnrealMotionsModule::AddCommandsToList(TSharedPtr<FBindingContext> MainFrameContext)
{
